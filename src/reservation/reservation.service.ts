...
export class ReservationService {
    ...
    async getAvailableRoom(getAvailableRoomDto: GetAvailableRoomDto) {
        ...
        subQuery.select('reser.roomId')
            .where('(:startTime BETWEEN reser.startTime AND reser.endTime)', { startTime: newStartTime })
            .orWhere('(:endTime BETWEEN reser.startTime AND reser.endTime)', { endTime: newEndTime })
            .orWhere('((reser.startTime BETWEEN :startTime AND :endTime) AND (reser.endTime BETWEEN :startTime AND :endTime))', { startTime: newStartTime, endTime: newEndTime })
    ...
    async addVehicleReservation(
        startTime: Date, endTime: Date, roomIds: number[], userId: number
    ) {
        const now = new Date();
        if (
            ...
            throw new BadRequestException('Your selected time is in the past, please reselect.')
        }
        ...
        if (checkReservation) {
            throw new BadRequestException(`${checkReservation.room.name} room was booked.`)
        }
